...
def clean_existing_datasets():
    print("\n[*] Nettoyage des datasets existants...")
...
with app.app_context():
    # Supprimer uniquement les datasets (pas les dashboards/charts)
    datasets = db.session.query(SqlaTable).all()
    for dataset in datasets:
        print(f'Suppression dataset: {{dataset.table_name}}')
        db.session.delete(dataset)
    
    db.session.commit()
    print('Datasets nettoyés')
"
""".format(container=SUPERSET_CONTAINER)
    
    result = subprocess.run(clean_cmd, shell=True, capture_output=True, text=True)
    if result.returncode == 0:
        print("✅ Datasets nettoyés")
    else:
        print("⚠️  Nettoyage des datasets échoué")
